...
def cookie_length(policy_json, length):
	...
	if "cookie-settings" in  policy_json["policy"] :
		if check_value(policy_json["policy"]["cookie-settings"], "maximumCookieHeaderLength", length):
			return policy_json, False, "Error!. Same Cookie length is already configured"
		else:		
	...
	return policy_json, True, "Success!. Cookie length: "+ str(length)+" is configured"
...
def main():
    ...
    jData, result, msg = cookie_length(jData,value)
