...
# Function that configures the 'parameter' is allowed/not-allowed in the security policy
def parameter(policy_json, name, enabled):
    ...
    if "parameters" in policy_json["policy"] and isinstance(policy_json["policy"]["parameters"], list):
        ...
        if parameter_exists:
            # If the parameter exists, remove any potential '$action' attribute
            if policy_json["policy"]["parameters"][x].get("$action") == "delete" and enabled==True:
                del policy_json["policy"]["parameters"][x]["$action"]
                # Set the 'allowed' attribute according to the provided value
                policy_json["policy"]["parameters"][x]["allowed"] = enabled
                return policy_json, True, f"Success! Parameter '{name}' allowed by removing the '$action=delete' "
            # If the parameter is already configured, return with an error message
            if policy_json["policy"]["parameters"][x].get("allowed") == enabled:
                return policy_json, False, f"No Change! Parameter '{name}' is already set to {'enabled' if enabled else 'disabled'}"
            else:
                # Set the 'allowed' attribute according to the provided value
                policy_json["policy"]["parameters"][x]["allowed"] = enabled
        else:
            # If the parameter does not exist, add it to the 'parameters' list with the provided 'allowed' attribute
            policy_json["policy"]["parameters"].append({"name": name, "allowed": enabled})
    else:
        # If 'parameters' key does not exist, create it and add the specified parameter with the provided 'allowed' attribute
        policy_json["policy"]["parameters"] = [{"name": name, "allowed": enabled}]
    
    # Return the modified policy JSON, indicating a successful configuration
    return policy_json, True, f"Success! Parameter '{name}' set to {'enabled' if enabled else 'disabled'}"
...
def main():
    ...
    updated_policy_data, is_changed, message = parameter(policy_data, name, enabled)
