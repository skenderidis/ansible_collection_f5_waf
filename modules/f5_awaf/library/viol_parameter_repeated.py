...
# Function that configures the 'repeated' setting for a parameter in the security policy JSON
def parameter_repeated(policy_json, name, enabled):
    ...
    if "parameters" in policy_json["policy"] and isinstance(policy_json["policy"]["parameters"], list):
        ...
        if parameter_exists:
            # If the parameter exists, update its 'allowRepeatedParameterName' setting
            if policy_json["policy"]["parameters"][x].get("allowRepeatedParameterName") == enabled:
                # If the parameter is already configured with the specified status, return with an error message
                return policy_json, False, f"No Change! Parameter '{name}' was already set to {'enabled' if enabled else 'disabled'} for repeated entries"
            else:
                # Update the 'allowRepeatedParameterName' setting of the parameter
            ...
            policy_json["policy"]["parameters"].append({"name": name, "allowRepeatedParameterName": enabled})
    else:
        # If 'parameters' key does not exist, create it and add the specified parameter with the setting
        policy_json["policy"]["parameters"] = [{"name": name, "allowRepeatedParameterName": enabled}]

    # Return the modified policy JSON, indicating a successful adjustment
    return policy_json, True, f"Success! Parameter '{name}' is set to {'enabled' if enabled else 'disabled'} for repeated entries"
...
def main():
    ...
    updated_policy_data, is_changed, message = parameter_repeated(policy_data, name, enabled)
